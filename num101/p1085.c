...
但是津津如果一天上课超过八个小时就会不高兴，而且上得越久就会越不高兴。假设津津不会因为其它事不高兴，并且她的不高兴不会持续到第二天。请你帮忙检查一下津津下周的日程安排，
...
int main()
{
    int school[7],mother[7],arr[7]={0},sum,n=0;
    for(int i=0;i<7;i++)
    {
        scanf("%d %d",&school[i],&mother[i]);
        sum=school[i]+mother[i];
        if(sum>8)
        {
            arr[i]=sum-8;
            n++;
